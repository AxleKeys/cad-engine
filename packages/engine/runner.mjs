...
// `api.joint()` — the fifth declaration (spine S4 / create-language-plan.md §h C1).
// ⚠ IMPORTED, NOT MIRRORED, unlike declareAnchor/declareEntity below — and that is a
// deliberate break with this file's own pattern. A mirrored anchor validator that drifts
// produces two MESSAGES; a mirrored joint validator that drifts produces two `data.joints`
// SETS, because this declaration writes. app/cadWorker.ts imports the same module.
import { declareJoint, resolveDeclaredJoints } from "../../app/lib/jointDeclare.js";
...
// Each part's solid count, taken in the same live-shape window (app/lib/connectivity.ts). A fact on
// every build; only certify's sweep (scripts/headless/sweep.mjs) compares it with the code as saved.
import { countPieces } from "../../app/lib/connectivity.js";
 ...
 *  `collectJoint` receives api.joint() declarations (t46-api-joint), APPENDED for the same
 *  reason — every positional caller of this function stays correct. */
export function buildApi(r, collect, assets, collectEntity, collectJoint) {
  return {
    ...
    // Declared degree of freedom — the program saying its own rig (spine S4). Costs no
    // geometry: it records a typed DOF against an anchor this same code declares, and the
    // build merges it into `data.joints` as a DERIVED set. The contract, the validation and
    // the ball expansion live in app/lib/jointDeclare.ts, shared with cadWorker rather than
    // mirrored here, because this declaration writes.
    joint: declareJoint.bind(null, collectJoint),
 ...
 * { ok, error?, parts: [{ name, bbox, obb, volume, vertexCount, solids, smallestSolidMm3? }], overall, elapsedMs }
 ...
 *
 * opts.pieceVolumes: a Set of part names. Every part reports `solids`, the number of solids its
 * shape holds (app/lib/connectivity.ts countPieces; null when the walk threw). A part whose name is
 * in this Set, holding 2 to PIECE_VOLUME_CAP solids, also reports `smallestSolidMm3`. When the Set
 * is absent no volume is read: push, /validate, /execute and drawings pay the walk alone. Only
 * certify's sweep (scripts/headless/sweep.mjs) passes it.
 */
export async function runModel(code, opts = {}) {
  ...
  try {
    ...
    // ⚠ An ARRAY, not a Map: a duplicate joint name is REFUSED rather than overwritten
    // (resolveDeclaredJoints). Last-wins is right for an anchor, which is a handle; a joint
    // owns PARTS, and dropping the first declaration would leave them moving with nothing.
    const jointDecls = [];
    const rawApi = buildApi(r, a => anchorMap.set(a.name, a), opts.assets, e => entityMap.set(e.id, e), d => jointDecls.push(d));
    ...
    // ⚠ `declaredJoints: null`, not `[]` — the soundness/swept vocabulary of this file. A build
    // that did not finish CANNOT TELL what its code declares, and `[]` is the claim "it
    // declares none", which mergeDeclaredJoints would act on by deleting the model's rig.
    if (!result) return { ok: false, error: "main() returned nothing", parts: [], anchors: [], declaredJoints: null, elapsedMs: Date.now() - t0 };

    // ⭐ Resolved AFTER main() returned, because a joint may name an anchor declared further
    // down the file — and inside this try, so an unresolvable anchor or a declaration cycle is
    // a legible BUILD ERROR rather than a joint that quietly is not there. The same shared
    // module app/cadWorker.ts calls: the tab and this headless door cannot report two
    // different rigs for one program.
    const declaredJoints = resolveDeclaredJoints(jointDecls, [...anchorMap.values()]);
    // ⚠⚠ `declaredJoints`, NEVER `joints`, and the name is load-bearing rather than verbose.
    // `opts.joints` on this same function means the model's STORED rig, handed in so the
    // swept-collision pass can sweep it. This is the opposite direction — what the code just
    // DECLARED, on its way out to be merged into that stored rig. One word for two concepts is
    // how a reader comes to pass one where the other belongs
    // [[feedback_shared_word_is_not_shared_concept]], and here that would sweep a model
    // against the joints it declares instead of the joints it has.
    ...
    const parts = namedParts.map(({ name, shape }) => {
      ...
      const ob = bb
        ...
        : null;
      // How many solids this part is (app/lib/connectivity.ts), read after mesh() while the shape is
      // alive. Plain numbers only: the geometry service serializes this object. A piece volume is
      // read only for the names a sweep hands over in opts.pieceVolumes.
      const pc = countPieces(oc, shape.wrapped, { volumes: !!opts.pieceVolumes?.has?.(name) });
      return {
        name, bbox: bb, obb: ob, volume, vertexCount: (mesh.vertices?.length ?? 0) / 3,
        solids: pc.solids,
        ...(pc.smallestSolidMm3 != null ? { smallestSolidMm3: pc.smallestSolidMm3 } : {}),
        mesh,
      };
    });
    ...
    return {
      ...
      // The api.joint() declarations, resolved against THIS build's anchors (spine S4).
      // ⭐ An ARRAY here is a claim the consumer acts on: `[]` means "this code declares no
      // joints" and the merge deletes the derived set accordingly, while an ABSENT/null field
      // means "this build cannot tell" and the merge must do nothing. Keep the two tellable
      // apart at every hop — services/geometry/server.mjs builds its /validate response
      // field-by-field, which is exactly how `anchors` was silently dropped for months.
      declaredJoints,
    ...
    return {
      ...
      parts: [], anchors: [], declaredJoints: null, elapsedMs: Date.now() - t0,
    };
  }
}
