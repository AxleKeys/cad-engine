...
pictures. `certify_model` at the ends of the ranges closes it.
...
**Prove the envelope with `check_dimensions` before any panel exists** — a single box at the
stated outside dimensions. A wrong envelope found after the joinery is written wastes every
part above it, and the Prime Directive above is exactly the error that hides until then.
