...
Cutting each stacked body separately is fragile — any one of those cuts can fail. Fuse the
stack first, then cut once; the cutter must still overshoot. The fragile-vs-reliable pair is
in `pipeline-code-agent` § Sequential boolean cuts.
...
The write is `push_model_code` (arguments in `MasterSkill`). For a repair, the `rationale` should say **what broke and why this fix is correct** — the next reader inherits the bug's history, not your session.
