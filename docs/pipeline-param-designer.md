...
How to decide what the user should control, what should be derived, what should be hidden, and how to keep the model valid as values change. This becomes the `param_schema` you pass to `push_model_code`.

⚠ **The schema does not create controls — the code does.** The `// [min:max]` annotation on a
const **declares** a control; the schema row **decorates** one (label, group, unit, buyer
visibility). Studio resolves the control list from the annotated consts and drops schema rows
that match no const — those render as drift to be fixed, never as sliders. A schema row without
an annotated const is not a slider; it is a bug report.
...
`param_schema` is a **flat array** of parameter objects — pass it to `push_model_code`. It is
**optional decoration**: the annotated consts in your code already declare the controls, and a
model with good annotations and no schema is fully adjustable. The schema is how you give those
controls a label, a group, a unit and buyer visibility. Each item:
...
⚠ **A row that matches no annotated const is not silently ignored — it is surfaced as drift.**
Studio drops it from the control list and renders it in the Create menu as a drift row with a
Fix button. So a typo in `name` does not cost you a slider quietly; it costs you a slider and
files a visible defect against the model.
