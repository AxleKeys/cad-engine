...
⭐ **The strongest version of that is to declare the whole joint in the code**, with
`api.joint("door_right", { type, parts, anchor, range })` — see `api.joint()` in the replicad
API reference. Then there is no pivot to drift and no second place the rig lives: the build
re-derives it from the parameters every time. Such joints are marked `declared_by` in
`list_joints`, and `update_joint` / `remove_joint` REFUSE on them, because an edit made through
a tool would be silently overwritten by the next `push_model_code`. Change the code, push, and
the rig follows. Joints you add here with `add_joint` sit alongside untouched and win any name
collision. A model's rig can mix both — a code-declared door and a hand-placed drawer.
...
5. **`set_frame_notes`** — the frame's on-screen TEXT beyond its label. ⭐ One field decides the
   ...
   ⚠⚠ **A frame is POOLED, so these notes play on EVERY shot that uses it.** To change what ONE
   SHOT of a cut says — which is where a directed beat's caption lives — that is `set_entry_notes`
   below, and a caption's id sent here is refused with the address to use instead.
6. **`set_entry_notes`** — the same thing for **one shot of one cut**, and **the only way to edit
   what a directed shot says**. A Direction's `say` compiles to a caption on the timeline **entry**,
   not on the frame, because one frame is shared by every shot that plays the same scene — so two
   shots of one scene can say different things. `list_frames` shows it as each entry's derived
   `caption`, beside the `note_ids` this tool takes back; address it by `video_id` + `entry_index`
   (the 0-based position in that cut's `sequence`), and send the `frame_id` you read to have a cut
   that changed under you **refused** instead of captioned on the wrong shot. Same two kinds as
   above, same `subject` rule, same opt-in burn.
   ⚠⚠ A shot with captions of its own **overrides** the frame's notes for that shot only — the frame
   keeps them, and every other shot and the walkthrough still show them. `notes: []` clears the
   shot's and it inherits the frame's again. Build-step notes are **never** overridden.
   ⭐ Editing a caption makes it **yours**: the next `direct_video` re-apply of that direction
   **adopts your sentence** and still re-derives the shot's camera. Clearing one is not sticky in
   the same way — a re-apply of a beat that still `say`s something writes a fresh caption.
7. **Arrange** — `reorder_frames` (the pool), `set_interactive` (the click-through order),
...
8. **`render_video`** — queues the MP4. **Poll `get_job_status`** → `result.download_url`.
