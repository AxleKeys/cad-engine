...
Complete, runner-verified models (each built at its parameter extremes: every parameter at
its min and max, and each pair at opposite ends). The lessons of this file: boolean ORDER (fuse solids first, cut last, finish last), cutters that
...
**Specific to this one:** read the part count in each verdict — a boolean that consumed the
wrong solid shows up as a changed part count immediately, whereas the same mistake found after
filleting is a rewrite.
