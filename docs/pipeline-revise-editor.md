...
The function list and the allowed return values are stated once, in `pipeline-code-agent`
§ Function Contract; the authoritative function surface is `api-replicad` (plus
`api-occ-advanced` for OCC-level names). This file used to carry its own copy of the
destructure — it had silently lost `makeBaseBox` and stayed that way for months, which is
the whole argument for one list.
...
The write is `push_model_code` (arguments in `MasterSkill`); it returns the build verdict — read it.

For a revision, the `rationale` should say **what changed and why** — it overwrites the model's current reasoning, so carry forward anything from the old rationale that still holds. It is the only record of intent that survives you.
