...
1. `get_active_context` — where the user is, which model is open, and your next moves. (Never ask the user; ask Axle.)
2. Simple part: `get_skill_pack({ section: "first-model" })` is the whole path. Anything more: `get_skill_pack({ section: "api-replicad" })`, the full API you write against.
...
| **REFINEMENT + SIGN-OFF** | Fillets/chamfers LAST (they are fragile) | `certify_model` at the ends of the ranges · `capture_turntable` sign-off · rationale updated |
...
**A model is parametric when it BUILDS ACROSS ITS RANGES, not when it has consts.**
...
That is why REFINEMENT ends with it; a middle value that matters is yours to build (set it with update_parameters and read the verdict). And the axis law rides every
...
| One simple part, from an empty file to a checked build — the short path, excerpted from the sections below | `first-model` |
...
| The user attached anything — an image, a DAE / STL / DXF / STEP / SVG / CSV file, a spec doc or a client brief | `reading-references` |

Four sections still carry `pipeline-` names for historical reasons. They are **reference material for you**, not roles for anyone else — read them as craft, and ignore any instruction to emit JSON or hand work to another agent. Five more that were pure role prompts for that pipeline were retired on 2026-09-08 and are no longer served; what was worth keeping in them is `reading-references`, addressed to you. The table above is the whole list — a section not in it does not exist, and asking for one costs you a call.
...
### The same engine runs offline

`@axle-keys/cad-engine` is a free local runner for the same model code, published on npm:

```bash
npx -y @axle-keys/cad-engine build your-model.js
```

It builds with the same kernel and prints the same verdict — build status, per-part bounding
boxes, validator warnings — so you can iterate on geometry without a network round trip, and
run it in CI. The boundary is honest: it is **only** the build. It holds no projects,
assemblies, drawings, materials or persistence, and it cannot see anything you have stored in
Axle. When the geometry is right, the model still has to be written through `push_model_code`
for any of that to exist.
