...
⭐ **`api-replicad` is the authoritative function list — this file does not carry one.** The
runtime exposes **80** functions; an enumeration here would be a second, staler copy of that
section, and for months it was: a third copy in another pipeline file quietly lost a name
nobody noticed. Load `api-replicad` for the surface, and `api-occ-advanced` for the OCC-level
names (`getOC`, `cast`, `downcast`, `iterTopo`, `genericSweep`, `makeBSplineApproximation`,
`makeNonPlanarFace`, `makeOffset`, `localGC`, `GCWithScope`) — freeform surfaces, smooth lofts,
variable fillets, raw BRep.
...
- Use only functions documented in `api-replicad` / `api-occ-advanced`.
...
- **Choice parameters** (styles, modes): a numeric INDEX const carrying a `choices:` list — the grammar is in `pipeline-param-designer` § segmented. What matters here is the code: branch on the index (`roofStyle === 0`) or an array lookup, and **every branch must produce valid geometry** — certify_model builds only the first and last option, never the ones between, so a middle branch that breaks is not caught.
...
When a part is a GROUP of members that only need to look adjacent, return
`makeCompound([...])` rather than chaining `.fuse()` — booleans are the expensive part of a
build. Fuse only when the result must be one manufactured body.

⚠ Clip each member BEFORE compounding — intersecting a whole compound quietly keeps just one
member and returns a plausible, wrong shape. `api-replicad` § Compound & Multi-Shape has the
measured costs and the correct form.
...
- `param_schema` — an **optional array** of parameter objects that *decorates* the controls your code already declared (label, group, unit, buyer visibility). It does **not** create them: the `// [min:max]` annotation on a const is what makes a slider, and a schema row matching no const is dropped and shown as drift. See `pipeline-param-designer` for the item shape.
...
The push returns the build verdict — read it; it is the source of truth. Only a *pending* verdict (`buildPending`) needs a follow-up: `get_validation_report` in a few seconds, until its status is ok or error and it is not stale; never push again to find out. Never conclude "it worked" from a screenshot.
...
- Are there no invented methods — only functions documented in `api-replicad` / `api-occ-advanced`?
