...
Complete, runner-verified models. Every example builds cleanly at its parameter extremes
(each slider at its min and max, all at min and max, each pair at opposite ends). Imitate the *structure*:
...
parts) → joinery/features → refinement + sign-off. **Probes dominate this class — `measure`
...
proportions or support need an eye. `certify_model` at the ends of the ranges closes it.
...
**Specific to this one:** `measure` a shelf against a side to confirm they share a face rather
than overlap. A derivation error in the shelf-spacing math then surfaces at the pass that
...
valid geometry — certify_model builds only the first and last option, so a middle one is yours to check.
...
const main = ({ makeBaseBox, Sketcher }) => {
  ...
  const roofZ = t + size;
  // the front elevation is the XZ plane, where BOTH signs flip: the offset puts the plane at
  // world Y = -offset, and the extrusion runs toward -Y. Sketching at -size/2 and extruding
  // `size` therefore spans y = -size/2 … +size/2 — flush with the centred body. (Measured.)
  if (roofStyle === 0) {
