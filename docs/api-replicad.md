...
```js
// Second argument is an OFFSET along the plane's normal — NOT a world coordinate:
new Sketcher("YZ", d)   // plane at world X = +d ; extrude runs +X   → X: d … d+len
new Sketcher("XY", d)   // plane at world Z = +d ; extrude runs +Z   → Z: d … d+len
new Sketcher("XZ", d)   // plane at world Y = −d ; extrude runs −Y   → Y: −d … −d−len
                        // ⚠⚠ XZ FLIPS BOTH SIGNS. To occupy y = a … b, write
                        //    new Sketcher("XZ", -b).extrude(b - a)
new Sketcher(planeObject)   // also accepts a Plane object
```

⚠⚠ **XZ IS THE ODD ONE, AND BOTH ITS SIGNS FLIP — the offset's as well as the extrusion's.**
Measured 2026-09-11 by building all three at a **non-zero** offset (`.extrude(10)` on a closed
20×20 rectangle) and reading the bounding boxes:

| plane | call | measured X | measured Y | measured Z |
|---|---|---|---|---|
| YZ | `Sketcher("YZ", 30).extrude(10)` | **30 → 40** | 0 → 20 | 0 → 20 |
| XY | `Sketcher("XY", 30).extrude(10)` | 0 → 20 | 0 → 20 | **30 → 40** |
| XZ | `Sketcher("XZ", 30).extrude(10)` | 0 → 20 | **−40 → −30** | 0 → 20 |

⭐ **Measure at a non-zero offset or you cannot see this.** At `d = 0` the offset's sign is
invisible, so the plane looks like it sits where you asked and only the extrusion direction is
wrong. The rule was written twice from an offset-0 reading and came out wrong both times.

The XZ plane's normal is negative world Y (right-handed: X × Z = −Y), so the extrusion runs
*toward the viewer's front*, and the offset walks the plane the same way. The consequence:

```js
// To occupy y = 0 … 60 — MEASURED, both forms built:
new Sketcher("XZ", -60).extrude(60)   // ✅ Y:    0 →   60
new Sketcher("XZ",  60).extrude(60)   // ✗ Y: −120 → −60   — twice the offset, wrong side
```

The same normal is why `EdgeFinder.inPlane("XZ", offset)` selects an exterior face at world
`y` with `offset = -y`.

This was documented three ways over months — the table first said `+Y`; the correction said
`−Y` but kept a positive offset in its worked example, which places the body at **twice** the
intended distance on the wrong side. Two runtime lessons said "place the sketch at the negative
of the intended far Y face" the whole time, and those lessons were right. A model built on
either wrong version is displaced by its own depth — a shift no dimension check catches,
because every part is still the correct SIZE.
...
⭐ **`genericSweep` and the OCC-level surface are documented in `api-occ-advanced`**, not here:
`genericSweep`, `makeBSplineApproximation`, `makeNonPlanarFace`, `makeOffset`, `iterTopo`,
`cast`, `downcast`, `getOC`, `localGC`, `GCWithScope`. They are in the `api` object like
everything else — load that section when you need them rather than guessing a signature.
...
assembleWire(edgesOrWires)    // → Wire (joins an array of Edges/Wires into one)
```
...
⚠⚠ **The plane is an OBJECT, not a positional string.** Five of these six take a single
`planeConfig` object — `{ plane?, origin? }` — after their shape arguments. Passing a bare
`"XZ"` where the object belongs does **not** throw and does **not** warn: the argument is
ignored, the default XY plane silently stands, and the build returns a green verdict for a
solid on the wrong plane.

```js
sketchCircle(radius, planeConfig?)
sketchRectangle(xLength, yLength, planeConfig?)
sketchEllipse(xRadius?, yRadius?, planeConfig?)
sketchPolysides(radius, sidesCount, sagitta?, planeConfig?)   // ⚠ sagitta sits in slot 3
sketchRoundedRectangle(width, height, r?, planeConfig?)        // r may be { rx, ry }
sketchHelix(pitch, height, radius, center?, dir?, lefthand?)   // ⚠ no planeConfig; height
                                                               //   BEFORE radius
// planeConfig: { plane?: PlaneName | Plane, origin?: Point | number }
sketchCircle(10, { plane: "XZ", origin: 40 })
```

**Measured 2026-09-11**, `sketchCircle(10, …).extrude(5)`, bounding boxes from a build:

| call | measured | verdict |
|---|---|---|
| `sketchCircle(10, "XZ")` | Z **0 → 5** | ✗ built on the **default XY plane** — the string was ignored, and the build was green |
| `sketchCircle(10, { plane: "XZ" })` | Y **−5 → 0** | ✅ genuinely on XZ |
| `sketchCircle(10, { plane: "XZ", origin: 40 })` | Y **−45 → −40** | ✅ plane at world Y = −40 |

⭐ `origin` obeys the same law as `new Sketcher`: on XZ, `origin: d` puts the plane at world
Y = **−d** and the extrusion runs −Y. The two APIs do not disagree — see the Sketcher section.
...
**Write the corners as the same expressions the geometry uses** — the same law as anchors
...
## Movement joints — `api.joint()`

Declares a **degree of freedom**: this part swings about that hinge, from here to there. The
model's program says what it can DO, the way it already says what shape it is. Costs no
geometry — a joint re-poses named parts at render time and never rebuilds anything.

```js
const main = (api) => {
  const width = 600;   // [400:900]
  const depth = 400, t = 18;

  api.anchor("hinge_right", { origin: [width - 30, depth, 0], axis: [0, 0, 1] });

  api.joint("door_right", {
    type:   "revolute",         // "revolute" (swings) | "prismatic" (slides) — those two
    parts:  ["Door"],           // build-part names this joint moves
    anchor: "hinge_right",      // REQUIRED — an api.anchor() this same code declares
    range:  [0, 110],           // degrees (revolute) / mm of travel (prismatic)
    label:  "Right door",       // OPTIONAL — defaults to the name
    axis:   [0, 0, 1],          // OPTIONAL — omit to inherit the anchor's, re-derived every build
    parent: "carcass_tilt",     // OPTIONAL — this joint's motion composes on top of that one
    driven: { source: "door_left", ratio: -1 },   // OPTIONAL — value computed from another joint
  });

  return [{ name: "Carcass", shape: body }, { name: "Door", shape: door }];
};
```

**Write the range and the anchor as expressions over your parameters.** The joint is
re-derived on every build, so a hinge declared against `width - 30` follows `width`. That is
the whole reason to declare it here rather than typing it into a form: a measured pivot is
correct once, and a revolute joint DOUBLES that error at the open end.

- `anchor` is **required**, and there is no `pivot`. A joint declared in the program must not
  be able to freeze a coordinate — the build fills the fallback pivot in from the anchor's
  live origin every time. An anchor the code does not declare fails the build and names what
  it does declare.
- **Two types, and that is the whole list.** A **ball** joint is not a type: pass
  `dof: ["x","y","z"]` on a revolute and this emits one revolute per degree of freedom, all at
  the same anchor, chained — ids `<name>-x`, `<name>-y`, `<name>-z`, in application order, and
  the parts land on the deepest. A **cylindrical** joint is a revolute plus a prismatic on one
  axis: two calls. A **fixed** joint is what `api.anchor()` already is.
- `range` is required today. It is meant to be omittable for a continuous joint, and the
  record cannot express that yet, so an omitted range is refused rather than quietly turned
  into stops you did not write.
- Omit `axis` to inherit the anchor's direction, which re-derives with the parameters — the
  only way a hinge whose DIRECTION moves (a hatch on an adjustable roof) stays right. An
  anchor's axis points outward by mating convention, so negate it in the `api.anchor()` call
  if the swing comes out backwards.
- `parent` is STRUCTURE (a shoulder carries its elbow); `driven` is VALUES
  (`value = source × ratio + offset`, clamped). A joint may use either or both. A cycle in
  either, among the joints this program declares, fails the build.
- A joint may be declared **before** the anchor it names — the build resolves them together
  once `main()` has returned.

**These joints are re-derived, and they are the program's.** Each build replaces what the
previous one declared, so deleting the `api.joint()` call removes the joint. Joints added by
hand or by `add_joint` sit alongside untouched and always win a name collision. Going the
other way, `update_joint` and `remove_joint` **refuse** on a joint declared here and tell you
to edit the code — an edit made there would be silently overwritten by your next push. Its
resting value (`default`) and its region tag (`group`) stay editable, because this declaration
does not state them.
