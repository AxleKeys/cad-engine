...
answer at all. When you need to name the version — reporting a bug, pinning a run —
`npm view @axle-keys/cad-engine version` is the nearest thing: it reports the registry's
current latest, which is what `npx -y` resolves *unless* a release landed in between or the
cache is stale. Say which you read it from, rather than presenting it as the version that ran.
...
⚠ **A gauge only works INSIDE one model.** Interference is measured between the parts of a
single build, never across models, so the gauge and the thing it measures are parts of the same
`main`. If other guidance you have been given says to keep a model to a single part, that is
about how to structure a finished product; it is not about this instrument, and it does not
survive contact with a build whose parts have to be checked against each other.
...
`problems` — a list of objects shaped like this, sometimes with `unverified: true` alongside:

```json
{ "rule": "part_interference", "severity": "error",
  "message": "Parts 'A' and 'B' occupy 600,000mm³ of the SAME solid volume (measured
              intersection, not a bounding-box guess; boxes penetrate 60.0×100.0×100.0mm)." }
```

⚠ **The part names live only in the prose of `message`, single-quoted — there is no structured
part list on a problem.** So the name-filter that gauge rule 2 depends on is a small regex over
that string (`/'([^']+)'/g` pulls both names out). Write it before you write the sweep.
...
`{x,y,z}`, and where the repl gives `buildMs` it gives `elapsedMs` and `wallMs`):
...
**And sometimes there is no search.** Once you state the criterion properly, the arithmetic
occasionally settles it outright — one cold run's three openings came to 209.3mm against a
195mm requirement, and there was nothing to sweep. That is a good outcome, not a wasted setup:
the loop's job becomes *proof* rather than search. Run both instruments on the one candidate,
confirm they agree, and go to step 6.
