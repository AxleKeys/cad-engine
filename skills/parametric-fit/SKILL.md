...
On a client that renders ChatGPT Apps SDK or MCP Apps widgets (ChatGPT), it draws the card the
user sees: a 3D view, overall size, parameters as they now stand. Every other client gets one
line of text plus those fields and the parameters as structuredContent, and no image; there, say
where the model is rather than that they can see it.
...
lag the latest build by a few seconds. Do not skip the step silently: on a widget client the
card is how the user sees the fit rather than reading about it.
...
- `show_model` called (a card on a widget client, one line of text elsewhere), or
  `get_model_screenshot` where that is the tool you have, and a one-line account of what moved,
  in their units as well as millimetres if they read in inches.
