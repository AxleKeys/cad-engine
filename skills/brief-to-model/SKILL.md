...
On a client that renders ChatGPT Apps SDK or MCP Apps widgets (ChatGPT), it draws a card: a
3D view, the Studio screenshot or a dimension sketch, overall size, build status, parameters.
Every other client, Claude Code and Codex among them, gets one line of text (name, overall
size, build status, part count) plus those fields and the parameters as structuredContent, and
no image. There, tell the user where to look (the model in their Studio Project explorer)
rather than saying they can see it here. Pass the `model_id` create_model returned: with none,
it shows the model a Studio tab last reported open, which may be a different one.

Use `get_model_screenshot` only when **you** want to eyeball the geometry yourself; it
returns a raw image for your inspection and it lags the latest build. On a widget client,
`show_model` is the one for the human.
...
- `show_model` called: a card on a widget client, otherwise its one line of text and a pointer
  to the model in Studio.
