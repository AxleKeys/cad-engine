...
// The ONE exception decode (§3 F4, §8 K3 P1): an OCC throw is a bare pointer on 0.23 and a
// WebAssembly.Exception on 1.0.0, and this file's catch used to print the pointer verbatim
// into a verdict a customer's agent reads.
import { occErrorText } from "./occError.js";
 ...
 *
 *  ⭐ Evenly, not first-come-first-served. Every sweep point is equally part of a clean
 *  certification, and a greedy budget would let the early points spend the late
...
export function forEachSolid(
  ...
  let exp      = null;
  try {
    exp = new oc.TopExp_Explorer(wrapped, oc.TopAbs_ShapeEnum.TopAbs_SOLID, oc.TopAbs_ShapeEnum.TopAbs_SHAPE);
    while (exp.More()) {
...
function solidwiseCommonVolume(oc     , aw     , bw     )         {
  ...
  let ea      = null;
  try {
    ea = new oc.TopExp_Explorer(aw, ta.TopAbs_SOLID, ta.TopAbs_SHAPE);
    while (ea.More()) {
      const sa = ea.Current();
      let eb      = null;
      try {
        eb = new oc.TopExp_Explorer(bw, ta.TopAbs_SOLID, ta.TopAbs_SHAPE);
        while (eb.More()) {
...
function commonVolume(oc     , aw     , bw     )         {
  ...
  try {
    ...
    algo = new oc.BRepAlgoAPI_Common(aw, bw, progress);
    algo.Build(progress);
    ...
    props = new oc.GProp_GProps();
    oc.BRepGProp.VolumeProperties(shp, props, false, false, false);
...
/**
 * ⭐⭐ THE ONE PAIR MEASUREMENT, over RAW `TopoDS_Shape`s rather than replicad Shapes.
 *
 * Exported for app/lib/sweptCollision.ts (t7-sweptcheck), which asks this same question of the
 * same parts at a POSED location: its shapes come out of `BRepBuilderAPI_Transform` and are
 * therefore wrapped TopoDS_Shapes with no replicad object around them. Everything that makes the
 * answer trustworthy — the solid count, the multi-solid refusal, the OCC-pointer catch, the
 * contact/interference split at the floor — belongs to the pair, not to the pose, so it is stated
 * HERE once and called twice. A second copy over there is exactly the drift guard S27 and
 * t4-overlap-truth exist to prevent [[feedback_three_registries]].
 *
 * ⚠ `floor` defaults to INTERFERENCE_VOLUME so a caller cannot accidentally invent a second
 * threshold for what counts as shared volume.
 */
export function measureWrappedPair(
  oc     ,
  a                                ,
  b                                ,
  floor         = INTERFERENCE_VOLUME,
)              {
  const counts           = [];
  for (const side of [a, b]) {
    if (!side?.wrapped) return { kind: "unverified", reason: `part '${side?.name ?? "?"}' did not expose a B-rep shape to measure` };
    const n = countSolids(oc, side.wrapped);
    if (n === 0) {
  ...
  try {
    const volume = solidwiseCommonVolume(oc, a.wrapped, b.wrapped);
    ...
    // ⚠ This reason used to read "the common-volume boolean failed (OCC exception pointer
    // 8522992)" — an address, inside the degrade path F3 already calls too quiet. `oc` is
    // right here, so the decode is free (app/lib/occError.ts, §8 K3 P1).
    return { kind: "unverified", reason: `the common-volume boolean failed (${occErrorText(err, oc)})` };
  }
}

/** The build-path shape of the same call: an InterferenceEntry carries a replicad Shape, and the
 *  B-rep is one dereference inside it. No logic of its own — see measureWrappedPair above. */
function measurePair(oc     , a                   , b                   , floor        )              {
  return measureWrappedPair(oc, { name: a.name, wrapped: a.shape?.wrapped }, { name: b.name, wrapped: b.shape?.wrapped }, floor);
}
