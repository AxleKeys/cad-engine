...
// (no enums, no namespaces, no parameter properties). Both imports below are siblings in
// app/lib and carry their `.ts` extension for exactly that reason.
//
...
// The ONE exception decode (§3 F4, §8 K3 P1) — see errText below for why this file was the
// sixth throw site and the one F4's list of five never contained.
 ...
 *  every sweep point is equally part of a clean certification, and a greedy
 ...
 *
 *  ⭐ AND IT USED TO STOP AT THE POINTER. Every caller here already holds `oc`, so the decode
 *  was one call away and this file printed `OCC exception pointer 8522992` into a coverage
 *  finding instead — the sixth site of §3 F4's six, and the one F4's own list never had.
 *  Now delegated to app/lib/occError.ts, which also owns 1.0.0's WebAssembly.Exception
 *  branch, so the browser verdict and the headless one cannot diverge (§8 K3 P1). */
const errText = (e     , oc     )         => occErrorText(e, oc);
 ...
 */
export function signedVolume(oc     , solid     , onlyClosed         )       {
  let props      = null;
  try {
    ...
    oc.BRepGProp.VolumeProperties(solid, props, !!onlyClosed, false, false);
    ...
    return { ran: false, reason: errText(e, oc) };
  } finally {
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      findings.push({
        ...
        severity: "warning",
        unverified: true,
        message: `Part '${entry.name}' could not be explored for solids (${errText(e, oc)}), so it was not checked.`,
      });
      continue;
    }
    if (n === 0) {
    ...
    try {
      ...
      findings.push({
        ...
        severity: "warning",
        unverified: true,
        message: `Part '${entry.name}' stopped mid-check (${errText(e, oc)}); ${measured} solid(s) had been read across this build at that point.`,
      });
    }
  }
...
function checkOneSolid(
  ...
  // refuses is a poor thing to gate a customer's build on even when it is right.
  // ⚠ THE PARENTHETICAL HERE FLIPPED WITH THE BUMP and is therefore deleted rather than kept:
  // it read "`BRepCheck_Status` has ZERO occurrences in replicad_single.d.ts", true on 0.23 and
  // FALSE on 1.0.0, which exports 8. The kernel can now name a reason. That does NOT reopen the
  // decision — (a) and (b) above are the measurements that closed it, and neither moved — but a
  // reopening would now have a new fact to work with [[feedback_stale_reference_not_always_inert]].
  //
